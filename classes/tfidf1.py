...
tfidf_model1 = cwd / 'Files' / 'tfidf_model.pickle'
tfidf_matrix1 = cwd / 'Files' / 'tfidf_matrix.pickle'
document_id_mapping1 = cwd / 'Files' / 'document_id_mapping.pickle'
...
class TfidfEngine1:
    ...
    def save_model(self, documents):
        with open(tfidf_model1, 'wb') as f_model:
            pickle.dump(self.tfidf_model, f_model)
        with open(tfidf_matrix1, 'wb') as f_matrix:
            pickle.dump(self.tfidf_matrix, f_matrix)
        with open(document_id_mapping1, 'wb') as f_mapping:
            pickle.dump({doc['id']: doc['text'] for doc in documents}, f_mapping)

    def load_model(self):
        with open(tfidf_model1, 'rb') as f_model:
            self.tfidf_model = pickle.load(f_model)
        with open(tfidf_matrix1, 'rb') as f_matrix:
            self.tfidf_matrix = pickle.load(f_matrix)
        with open(document_id_mapping1, 'rb') as f_mapping:
