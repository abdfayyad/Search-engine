...
def Serve():
    if request.method == "GET":
        return render_template("temp_embedding.html")
    elif request.method == "POST":
        response = requests.post("http://localhost:5013/query", json={"input": request.form.get("input"), "dataset": request.form.get("dataset")})
        results = response.json().get("results")
        return render_template("temp_embedding.html", results=results, query=request.form.get("input"), count=len(results))
