...
_word_embedding_model = cwd / 'Files_embedding' / 'word_embedding_model.pickle'
_document_vectors = cwd / 'Files_embedding' / 'document_vectors.pickle'
_document_id_mapping_vector = cwd / 'Files_embedding' / 'document_id_mapping_vector.pickle'

class WordEmbeddingEngineAntique:
    ...
    def save_model(self):
        with open(_word_embedding_model, 'wb') as f_model:
            pickle.dump(self.word_embedding_model, f_model)
        with open(_document_vectors, 'wb') as f_vectors:
            pickle.dump(self.documents_vectors, f_vectors)
        with open(_document_id_mapping_vector, 'wb') as f_mapping:
            pickle.dump(self.document_id_mapping, f_mapping)

    def load_model(self):
        with open(_word_embedding_model, 'rb') as f_model:
            self.word_embedding_model = pickle.load(f_model)
        with open(_document_vectors, 'rb') as f_vectors:
            self.documents_vectors = pickle.load(f_vectors)
        with open(_document_id_mapping_vector, 'rb') as f_mapping:
